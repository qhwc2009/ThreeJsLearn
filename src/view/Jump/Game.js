...
export default function Game (dom) {
  ...
  this.camera = new THREE.OrthographicCamera(width/ -80,
    width/ 80,
    height / 80,
    height / -80,
    0.1, 5000);
...
Object.assign(Game.prototype, {
  // 拿到当前的物体大小指数
  getCubeSizeIndex: function() {
    return Math.pow(0.8, Math.floor(this.cubeIdx / 5));
  },
  // 拿到距离指数
  getDisIndex: function() {
    return Math.pow(1.2, Math.floor(this.cubeIdx / 5));
  },
  ...
  createCube: function (){
    ...
    const adjustCubeZ = this.config.cubeZ * this.getCubeSizeIndex();
    const adjustCubeX = this.config.cubeX * this.getCubeSizeIndex();
    const adjustCylinderRadius = this.config.cylinderRadius * this.getCubeSizeIndex();

    var geometry = cubeType === 'cube' ?
    new THREE.CubeGeometry(adjustCubeX, this.config.cubeY, adjustCubeZ):
    new THREE.CylinderGeometry(adjustCylinderRadius, adjustCylinderRadius, this.config.cylinderHeight, 100);
    var color = colorList[this.cubeIdx % colorList.length];
    var material = new THREE.MeshLambertMaterial( { color: color } );
    var mesh = new THREE.Mesh(geometry, material);

    // 产生随机图形
    if (this.cubes.length){
      const currentDisIndex = this.getDisIndex();

      var dis = this.getRandomValue(this.config.cubeMinDis * currentDisIndex, this.config.cubeMaxDis * currentDisIndex);
      var lastcube = this.cubes[this.cubes.length - 1];
      if (relativePos === 'zDir'){
        if (cubeType === 'cube'){
          if (lastcube.geometry instanceof THREE.CubeGeometry)
            // 方体 -> 方体
            mesh.position.set(lastcube.position.x, lastcube.position.y, lastcube.position.z - dis - adjustCubeZ );
          else  // 方体 -> 圆柱体
            mesh.position.set(lastcube.position.x, lastcube.position.y, lastcube.position.z - dis - adjustCylinderRadius - adjustCubeZ / 2);
        } else {
          if (lastcube.geometry instanceof THREE.CubeGeometry)
             // 圆柱体 -> 方体
             mesh.position.set(lastcube.position.x, lastcube.position.y, lastcube.position.z - dis - adjustCylinderRadius - adjustCubeZ / 2);
            ...
            // 圆柱体 -> 圆柱体
             mesh.position.set(lastcube.position.x, lastcube.position.y, lastcube.position.z -  dis - adjustCylinderRadius * 2 );
        }
      } else {
        if (cubeType === 'cube'){
          if (lastcube.geometry instanceof THREE.CubeGeometry)
            // 方体 -> 方体
            mesh.position.set(lastcube.position.x + dis + adjustCubeX, lastcube.position.y, lastcube.position.z);
          else  // 方体 -> 圆柱体
            mesh.position.set(lastcube.position.x + dis + adjustCubeX / 2 + adjustCylinderRadius, lastcube.position.y, lastcube.position.z);
        } else {
          if (lastcube.geometry instanceof THREE.CubeGeometry)
             // 圆柱体 -> 方体
             mesh.position.set(lastcube.position.x + dis + adjustCylinderRadius + adjustCubeX / 2 , lastcube.position.y, lastcube.position.z);
          else
            // 圆柱体 -> 圆柱体
             mesh.position.set(lastcube.position.x + dis + adjustCylinderRadius * 2, lastcube.position.y, lastcube.position.z);
        }
      }
    } else {
  ...
  _onwindowResize: function (){
    this.camera.left = this.dom.clientWidth / -80;
    this.camera.right = this.dom.clientWidth / 80;
    this.camera.top = this.dom.clientWidth / 80;
    this.camera.bottom = this.dom.clientWidth / -80;
    this.camera.updateProjectionMatrix();

    this.renderer.setSize(this.dom.clientWidth, this.dom.clientWidth);
  },
  ...
  _onMouseUp: function (){
    ...
    if (this.jumper.position.y >= this.config.jumpHeight / 2){
      ...
      if (type === 1){
        ...
        // 成功降落
         this.score += (1 + Math.floor(this.cubeIdx / 5));
         this.xspeed = 0;
         this.yspeed = 0;
         this.jumper.scale.y = 1;
         this.jumper.position.y = this.config.jumpHeight / 2;
    ...
    if (fromType === 'cube'){
       if (toType === 'cube'){
           if ( fromPosition.x === toPosition.x ){
             ...
             d1 = Math.abs(fromPosition.z - adjustCubeZ / 2);
             d2 = Math.abs(toPosition.z + adjustCubeZ / 2);
             d3 = Math.abs(toPosition.z);
             d4 = Math.abs(toPosition.z - adjustCubeZ / 2);
           } else {
             // x 方向
             d = Math.abs(position.x);
             d1 = Math.abs(fromPosition.x + adjustCubeX / 2);
             d2 = Math.abs(toPosition.x - adjustCubeX / 2);
             d3 = Math.abs(toPosition.x);
             d4 = Math.abs(toPosition.x + adjustCubeX / 2);
           }
       } else {
         if ( fromPosition.x === toPosition.x ){
           // -z 方向
           d = Math.abs(position.z);
           d1 = Math.abs(fromPosition.z - adjustCubeZ / 2);
           d2 = Math.abs(toPosition.z + adjustCylinderRadius);
           d3 = Math.abs(toPosition.z);
           d4 = Math.abs(toPosition.z - adjustCylinderRadius);
         } else {
           // x 方向
           d = Math.abs(position.x);
           d1 = Math.abs(fromPosition.x + adjustCubeX / 2);
           d2 = Math.abs(toPosition.x - adjustCylinderRadius);
           d3 = Math.abs(toPosition.x);
           d4 = Math.abs(toPosition.x + adjustCylinderRadius);
         }
       }
    } else {
      if (toType === 'cube'){
        if ( fromPosition.x === toPosition.x ){
          // -z 方向
          d = Math.abs(position.z);
          d1 = Math.abs(fromPosition.z - adjustCylinderRadius);
          d2 = Math.abs(toPosition.z + adjustCubeZ / 2);
          d3 = Math.abs(toPosition.z);
          d4 = Math.abs(toPosition.z - adjustCubeZ / 2);
        } else {
          // x 方向
          d = Math.abs(position.x);
          d1 = Math.abs(fromPosition.x + adjustCylinderRadius);
          d2 = Math.abs(toPosition.x - adjustCubeX / 2);
          d3 = Math.abs(toPosition.x);
          d4 = Math.abs(toPosition.x + adjustCubeX / 2);
        }
